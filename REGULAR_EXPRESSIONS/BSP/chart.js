...
function svgChart(id,re,text,data){
    ...
    for (var i = 0; i <= 10; i++){
        svgAddHgridLine(svg,xorig,yorig-i*30,660,"darkgray",Math.floor(i * Math.floor(scale/10)));
    }
    ...
    for (var i = 1; i < data.length; i++){
        ...
        svgAddLegend(svg,730+xorig,lab[0],lab[1],50+(i*30));
    }

    if (curvesLegend != ""){
        svgAddLabel(svg,730+xorig,50,curvesLegend,"8pt");
    }
